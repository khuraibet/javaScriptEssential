const employees = [
    ...
    { id: 2, name: 'Alice Smith', age: 28, department: 'HR', salary: 45000, specialization: 'SalesForce' },
...
function findBySpecialization(skill) {
    const foundSkill = employees.find(employee => employee.specialization === skill);
    if (foundSkill) {
        document.getElementById('employeesDetails').innerHTML = `<p>${foundSkill.id}: ${foundSkill.name} - ${foundSkill.department} - $${foundSkill.salary}</p>`;
    } else {
